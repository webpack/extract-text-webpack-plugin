...
module.exports.pitch = function(request) {
  ...
  if(this[NS] === undefined) {
    ...
    return "";
  } else if(this[NS](null, options)) {
    if(options.omit) {
      this.loaderIndex += +options.omit + 1;

      request = request.split("!").slice(+options.omit).join("!");
      loaders = loaders.slice(+options.omit);
    }
    ...
    var publicPath = typeof options.publicPath === "string"
      ? options.publicPath
      : this._compilation.outputOptions.publicPath;

    var outputOptions = { filename: childFilename, publicPath: publicPath };

    var childCompiler = this._compilation
      .createChildCompiler("extract-text-webpack-plugin", outputOptions);
    ...
    childCompiler.runAsChild(function(err, entries, compilation) {
      ...
      if(resultSource) callback(null, resultSource);
      else callback();
    }.bind(this));
  }
};
