...
var ExtractedModule = require("./lib/Module");
var OrderUndefinedError = require("./lib/Error");
...
ExtractTextPlugin.prototype.mergeNonInitialChunks = function(
  chunk,
  intoChunk,
  checkedChunks
) {
...
ExtractTextPluginCompilation.prototype.addModule = function(
  identifier,
  originalModule,
  source,
...
ExtractTextPluginCompilation.prototype.addResultToChunk = function(
  identifier,
  result,
  originalModule,
  extractedChunk
...
ExtractTextPlugin.prototype.renderExtractedChunk = function(chunk) {
  ...
  chunk.modules.forEach(function(module) {
    ...
    source.add(this.applyAdditionalInformation(
      moduleSource,
...
function ExtractTextPlugin(options) {
  ...
  if(isString(options)) {
    ...
    validateOptions(require('./lib/plugin.json'), options, 'Extract Text Plugin');
  }
...
ExtractTextPlugin.prototype.applyAdditionalInformation = function(src, info) {
  if(info) {
...
ExtractTextPlugin.prototype.extract = function(options) {
  ...
  if(
    ...
    validateOptions(require('./lib/loader.json'), options, 'Extract Text Plugin (Loader)');
  }
...
ExtractTextPlugin.extract = ExtractTextPlugin.prototype.extract
  .bind(ExtractTextPlugin);
  ...
  compiler.plugin("this-compilation", function(compilation) {
    ...
    compilation.plugin("normal-module-loader", function(loaderContext, module) {
      loaderContext[NS] = function(content, opt) {
        ...
        module[NS] = { content: content, options: opt || {} };
        ...
        if(extractedChunk.modules.length) {
          extractedChunk.modules.sort(function(a, b) {
            if(!options.ignoreOrder && isInvalidOrder(a, b)) {
              compilation.errors
                .push(new OrderUndefinedError(a.getOriginalModule()));

              compilation.errors
                .push(new OrderUndefinedError(b.getOriginalModule()));
            }
          ...
          function getPath (format) {
            return compilation
              .getPath(format, {chunk: chunk})
              .replace(
